...
class CarNeuralNetwork:
    ...
    def mutate(self, noise: float, learn_rate: float, curr_fitness: Union[float, None]):
        if self.prev_weights is None or curr_fitness is None or learn_rate == 0:
            # random
            self.weights += np.random.normal(loc=0, scale=noise, size=self.weights.shape)
            self.weights2 += np.random.normal(loc=0, scale=noise, size=self.weights2.shape)
            self.weights3 += np.random.normal(loc=0, scale=noise, size=self.weights3.shape)
            self.weights4 += np.random.normal(loc=0, scale=noise, size=self.weights4.shape)
            return
        ...
        self.weights += learn_rate * np.sign(dfitness) * dweights * np.random.normal(loc=1, scale=noise, size=dweights.shape)
        self.weights2 += learn_rate * np.sign(dweights) * dweights2 * np.random.normal(loc=1, scale=noise, size=dweights2.shape)
        self.weights3 += learn_rate * np.sign(dweights2) * dweights3 * np.random.normal(loc=1, scale=noise, size=dweights3.shape)
        self.weights4 += learn_rate * np.sign(dweights3) * dweights4 * np.random.normal(loc=1, scale=noise, size=dweights4.shape)
    ...
    def from_string(self, string: str):
        ...
        self.weights3 = np.array(json.loads(weights3)).reshape((self.H2_SIZE + 1, self.H3_SIZE))
        self.weights4 = np.array(json.loads(weights4)).reshape((self.H3_SIZE + 1, self.OUTPUTS_SIZE))
